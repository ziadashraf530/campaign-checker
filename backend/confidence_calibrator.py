...
class ConfidenceCalibrator:
    ...
    def calculate_boosts(
        ...
        """
        Calculate social media confidence adjustment and product-centric brand boost.
        
        Returns:
            tuple[float, float]: (social_adjustment, product_boost)
        """
        social_adjustment = 0.0
        product_boost = 0.0

        # Support both standard reference cluster labels and fallback labels
        is_product_or_logo = dominant_cluster_label in (
            "logo_refs", "drink_refs", "product_refs", 
            "Brand Logo & Focus", "Product & Drink Close-ups"
        ) or "Logo" in dominant_cluster_label or "Product" in dominant_cluster_label or "Drink" in dominant_cluster_label

        # 1. Social Screenshot Boost
        if is_social_media and max_sim >= 0.78:
            social_adjustment = 0.05
            if is_product_or_logo:
                social_adjustment += 0.03

        # 2. Product-Centric branding boost (only if competitor overlap is low)
        if is_product_or_logo and max_sim >= 0.80 and competitor_sim < 0.65:
            product_boost = 0.05
            if max_sim >= 0.86:
                product_boost += 0.02

        # 3. Precision safeguard: if competitor similarity is high, suppress all boosts
        if competitor_sim >= 0.65:
            social_adjustment = 0.0
            product_boost = 0.0

        return round(social_adjustment, 4), round(product_boost, 4)
